...
def generate_initial_population(pop_size, target_length):
    ...
    for _ in range(pop_size):
        individual = ''.join(random.choice(string.ascii_lowercase) for _ in range(target_length))
...
def mutate(individual, mutation_rate):
    ...
    for bit in individual:
        if random.random() < mutation_rate:
            mutated_individual += random.choice(string.ascii_lowercase.replace(bit, ''))
        else:
...
population_size = 500
target_string = 'def'
initial_individual = 'abcdefghijklmnopqrstuvwxyz'
mutation_rate = 0.001
